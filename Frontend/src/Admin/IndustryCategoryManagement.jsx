...
function IndustryCategoryManagement() {
  ...
  const loadCategories = async () => {
    try {
      const response = await api.get('/admin/industry-categories', { headers: authHeader('admin') });
  ...
  const save = async () => {
    try {
      ...
      setStatus('Industry categories updated successfully.');
    } catch (error) {
      setStatus('Failed to update industry categories.');
    }
  };

  return (
    <div>
      <div className="section-head"><h1>Industry Category Management</h1></div>
      <div className="white-card form-grid">
        <input value={value} onChange={(event) => setValue(event.target.value)} placeholder="Add industry category" />
        ...
        <div className="admin-modal-overlay" onClick={() => setEditingIndex(null)} role="button" tabIndex={0}>
          <div className="admin-modal category-edit-modal" onClick={(event) => event.stopPropagation()}>
            <div className="admin-modal-head category-edit-head">
              <h2><i className="fa-solid fa-pen-to-square" /> Edit Industry Category</h2>
...
export default IndustryCategoryManagement;
